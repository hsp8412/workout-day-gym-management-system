...
const Appointments = () => {
    ...
    const getAppointmentContent = (a) => {
        ...
        result[2] = <div>{"Timeslot: " + `${a.slot + offset}:00 - ${a.slot + offset + 1}:00`}</div>;
        return result;
    };
    ...
    return (
        <Container>
                ...
                <Col>
                    <Card>
                        <Card.Body>
                            <h3>Date</h3>
                            <hr/>
                            <Calendar onChange={handleCalendarSelect} value={value} locale="en"/>
                        </Card.Body>
                    </Card>
                </Col>
                <Col>
                    <Card>
                        <Card.Body>
                            ...
                            <hr/>
                            <ListGroup>
                                {coaches.map(c => <ListGroup.Item action
                                                                  key={c.coachId}
                                                                  active={c === coach}
                ...
                <Col>
                    <Card>
                        <Card.Body>
                            <h3>Timeslot</h3>
                            <hr/>
                            <ListGroup>{array.map(a => <ListGroup.Item action key={a} active={slot === a}
                                                                       disabled={occupiedSlots.indexOf(a) >= 0}
                                                                       onClick={() => setSlot(a)}>
                                {`${a + offset}:00-${a + offset + 1}:00`}</ListGroup.Item>)}
                            </ListGroup>
                        </Card.Body>
                    </Card>
                </Col>
            ...
            <Row className="p-3">
                <Button disabled={slot === -1}
                        onClick={handleSubmit}>Make an appointment</Button>
            </Row>
            <Card>
                <Card.Body>
                    ...
                    {appointments.map(a =>
                        <Card>
                            <Card.Body className="d-flex flex-row justify-content-between">
                                <div>
                                    {getAppointmentContent(a)}
                                </div>
                                <Button variant="danger" onClick={() => handleDelete(a)}>Cancel</Button>
                            </Card.Body>
                        </Card>)}
                </Card.Body>
            </Card>
